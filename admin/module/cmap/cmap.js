...
class CmapApp {
  constructor() {
    ...
    let observer = new MutationObserver((mutations) =>
      $(`#${canvas.canvasId} > div`).css("width", 0)
    );
  ...
  handleEvent() {
    ...
    let saveAsDialog = UI.modal("#concept-map-save-as-dialog", {
      onShow: () => {
        ...
        if (sessions.user) {
          KitBuild.getTopicListOfGroups(sessions.user.gids.split(",")).then(
            (topics) => {
              ...
              topics.forEach((topic) => {
                ...
                if (this.conceptMap) {
                  selected =
                    this.conceptMap.map.topic == topic.tid ? " selected" : "";
                  if (
                    selected == "" &&
                    CmapApp.topic &&
    ...
    topicDialog.setTopic = (topic = null) => {
      $("#assign-topic-dialog .cmap-topic").html(
        topic
          ? topic.title +
              `<span class="badge rounded-pill bt-deassign-topic-from-cmap ms-2 bg-danger" role="button" data-cmid="${topicDialog.cmid}">Remove</span>`
    ...
    let textDialog = UI.modal("#assign-text-dialog", {
      ...
      onShow: () => {
        ...
        if (textDialog.cmap.map.text)
          this.ajax
            .get(`contentApi/getText/${tid}`)
            .then((text) => textDialog.setText(text));
    ...
    textDialog.setText = (text = null) => {
      $("#assign-text-dialog .assigned-text").html(
        text
          ? `<span class="text-primary">${text.title}</span> <span class="badge rounded-pill bt-deassign-text-from-cmap ms-2 bg-danger" role="button" data-cmid="${textDialog.cmap.map.cmid}">Remove</span>`
          : `<em class="text-muted">Not specified.</em>`
      );
    };
    ...
    $("#concept-map-save-as-dialog").on("submit", (e) => {
      ...
      if (this.conceptMap && this.conceptMap.map.author != this.user.username) {
        let confirm = UI.confirm(
          `Do you want to save and <span class="text-danger">TAKE OWNERSHIP</span> of this concept map created by <code>${this.conceptMap.map.author}</code>?`
        )
          .positive(() => {
            proceedSave();
            confirm.hide();
        ...
        openPromise.push(
          new Promise((resolve, reject) => {
            try {
              ...
              if (!conceptMap.map)
                reject(
                  "Concept map data does not have ID specified. Please try pasting and applying the data from the toolbar instead of opening it."
                );
              resolve(
                Object.assign(conceptMap, {
                  cyData: KitBuildUI.composeConceptMap(conceptMap),
      ...
      if (openPromise.length)
        Promise.any(openPromise)
          ...
          .catch((error) => {
            ...
            UI.dialog(
              `<span class="fst-italic text-danger">The concept map data is invalid</span>. ${
                error.errors ? "<br>" + error.errors : ""
              }`,
              {
    ...
    $(".app-navbar .bt-export").on("click", (e) => {
      ...
      if (this.conceptMap && this.conceptMap.map)
        canvasData.map = this.conceptMap.map;
      else
        canvasData.map = {
          cmid: null,
          cmfid: null,
    ...
    $(".app-navbar").on("click", ".bt-assign-topic", (e) => {
      ...
      KitBuild.openConceptMap(cmid).then((cmap) => {
        ...
        if (cmap.map.topic) {
          this.ajax
            .get(`contentApi/getTopic/${cmap.map.topic}`)
            .then((topic) => {
              // console.log(topic)
    ...
    $("form.form-assign-search-topic").on("submit", (e) => {
      ...
      let perpage = parseInt(
        $("form.form-assign-search-topic .input-perpage").val()
      );
      let keyword = $("form.form-assign-search-topic .input-keyword").val();
      let page =
        !CmapApp.assignTopicPagination ||
        keyword != CmapApp.assignTopicPagination.keyword
          ? 1
          : CmapApp.assignTopicPagination.page;
      ...
      Promise.all([
        ...
        if (CmapApp.assignTopicPagination) {
          ...
          CmapApp.assignTopicPagination = Pagination.instance(
            "form.form-assign-search-topic .list-topic-pagination",
            count,
            perpage
          )
            .listen("form.form-assign-search-topic")
            .update(count, perpage);
        $(".dropdown-menu-teacher-map-list").addClass("show");
      });
    });

    $("#assign-topic-dialog .list-topic").on(
      "click",
      ".bt-assign-topic-to-cmap",
      (e) => {
        ...
        this.ajax
          .post("contentApi/assignTopicToConceptMap", {
            cmid: cmid,
            tid: tid,
          })
          .then((result) => {
    ...
    $("#assign-topic-dialog").on(
      "click",
      ".bt-deassign-topic-from-cmap",
      (e) => {
        let cmid = $(e.currentTarget).attr("data-cmid");
        this.ajax
          .post("contentApi/deassignTopicFromConceptMap", {
            cmid: cmid,
          })
          .then((result) => {
            console.log(result);
    ...
    $("form.form-assign-search-text").on("submit", (e) => {
      ...
      e.stopPropagation();
      let perpage = parseInt(
        $("form.form-assign-search-text .input-perpage").val()
      );
      let keyword = $("form.form-assign-search-text .input-keyword").val();
      let page =
        !CmapApp.assignTextPagination ||
        keyword != CmapApp.assignTextPagination.keyword
          ? 1
          : CmapApp.assignTextPagination.page;
      Promise.all([
        ...
        if (CmapApp.assignTextPagination) {
          ...
          CmapApp.assignTextPagination = Pagination.instance(
            "form.form-assign-search-text .list-text-pagination",
            count,
            perpage
          )
            .listen("form.form-assign-search-text")
            .update(count, perpage);
        $(".dropdown-menu-teacher-map-list").addClass("show");
      });
    });

    $("#assign-text-dialog .list-text").on(
      "click",
      ".bt-assign-text-to-cmap",
      (e) => {
        let cmid = $(e.currentTarget).parents(".item-text").attr("data-cmid");
        let tid = $(e.currentTarget).parents(".item-text").attr("data-tid");
        console.warn(cmid, tid);
        this.ajax
          .post("contentApi/assignTextToConceptMap", {
            cmid: cmid,
            tid: tid,
          })
          .then((result) => {
    ...
    $("#assign-text-dialog").on("click", ".bt-deassign-text-from-cmap", (e) => {
      let cmid = $(e.currentTarget).attr("data-cmid");
      this.ajax
        .post("contentApi/deassignTextFromConceptMap", {
          cmid: cmid,
        })
        .then((result) => {
          console.log(result);
  ...
  handleRefresh() {
    ...
    this.session.getAll().then((sessions) => {
      ...
      try {
        if (stateData.logger) {
          this.logger = CmapLogger.instance(
            stateData.logger.username,
            stateData.logger.seq,
            stateData.logger.sessid,
            this.canvas
          ).enable();
          if (CmapApp.loggerListener)
            this.canvas.off("event", CmapApp.loggerListener);
          CmapApp.loggerListener = this.logger.onCanvasEvent.bind(
            CmapApp.inst.logger
          );
...
CmapApp.populateTopics = (topics) => {
  ...
  topics.forEach((topic) => {
    ...
    if (topic.text)
      topicsHtml += `    <span class="badge rounded-pill bg-success ms-2">${topic.text} <i class="bi bi-file-text"></i></span>`;
    topicsHtml += `  </span>`;
  ...
  if (topicsHtml.length == 0)
    topicsHtml =
      '<em class="d-block m-3 text-muted">No topics found in current search.</em>';
...
CmapApp.populateAssignTexts = (texts, cmid) => {
  ...
  if (textsHtml.length == 0)
    textsHtml =
      '<em class="d-block m-3 text-muted">No texts found in current search.</em>';
