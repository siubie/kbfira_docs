...
class KitBuildApp {
  constructor() {
    ...
    if (typeof KitBuildLogger != "undefined") {
      this.logger = KitBuildLogger.instance(null, 0, null, canvas).enable();
  ...
  handleEvent() {
    ...
    this.contentDialog = UI.modal("#kit-content-dialog", {
      ...
      onShow: () => {
        ...
        let htmlText =
          this.contentDialog &&
          this.contentDialog.text &&
          this.contentDialog.text.content
    ...
    $("#bug-dialog .bug-options").on("click", ".item-bug", (e) => {
      if (this.bugDialog.node) {
        ...
        let dim = this.nodeCreateTool.calculateDimension(
          this.bugDialog.node.data()
        );
    ...
    $(".app-navbar").on("click", ".bt-feedback", () => {
      ...
      let feedbacklevel = parseInt(
        KitBuildApp.inst.kitMap.parsedOptions.feedbacklevel
      );
    ...
    $("#register-dialog").on("click", ".bt-register", (e) => {
      ...
      if (name.trim().length == 0) {
        ...
        KitBuildRBAC.register(name, username, password, rid, gid)
          .then((user) => {
            // console.log(user)
    ...
    switch (event) {
      case "action":
        ...
        if (data.start && data.end) {
          let textSelectionTool =
            this.canvas.canvasTool.tools.get("text-select");
    ...
    this.session.getAll().then((sessions) => {
      ...
      Promise.all(promises).then((maps) => {
        ...
        if (kitMap && !learnerMap)
          KitBuildApp.resetMapToKit(kitMap, this.canvas);
        if (kitMap) {
          try {
            if (stateData && stateData.logger) {
              // reinstantiate and enable logger
              KitBuildApp.inst.logger = KitBuildLogger.instance(
              ...
              KitBuildApp.loggerListener =
                KitBuildApp.inst.logger.onCanvasEvent.bind(
